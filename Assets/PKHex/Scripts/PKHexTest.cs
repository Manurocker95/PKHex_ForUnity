...
namespace PokemonLetsGoUnity
{
    ...
    public class PKHexTest : MonoBehaviour
    {
        ...
        {
            var savLoader = PLGU_PKHexUtils.InitializeGenericSavLoader(sav);
            ...
            if (savLoader != null)
            {
                ...
                var kvp = PLGU_PKHexUtils.LoadPKMFromPath(PK);//savLoader.LoadPKMFromPath(PK);
                if (kvp.Key)
                {
                    Debug.Log("Pokemon loaded from PK: " + PLGU_PKHexUtils.GetPokemonSpeciesNameInLanguage(kvp.Value, SystemLanguage.English));
                }
            }
        }
