...
{
    public class PLGU_PKHexSavLoader<T> where T : SaveFile
    {
        ...
        public virtual int GetPokemonInPartyWithMove(T savefile, string _moveName, string _pokemon = "")
        {
            ...
            foreach (PKM pokemon in party)
            {
                ...
                if (moves.Contains(_moveName))
                {
                    ...
                    {
                        if (PLGU_PKHexUtils.SpeciesStrings.Contains(_pokemon))
                        {
        ...
        {
            return pokemon != null ? PLGU_PKHexUtils.GetMoveNames(pokemon.Moves) : new string[1] { PLGU_PKHexUtils.L_AError };
        } 
